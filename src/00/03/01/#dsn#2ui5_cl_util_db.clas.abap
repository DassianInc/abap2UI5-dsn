CLASS /dsn/2ui5_cl_util_db DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS /dsn/2ui5_cl_util_db IMPLEMENTATION.



  METHOD delete_by_handle.

    DELETE FROM /dsn/2ui5_t_91
        WHERE
           uname = @uname
            AND handle = @handle
            AND handle2 = @handle2
  ...
  METHOD load_by_handle.
    ...
    DATA lt_db TYPE STANDARD TABLE OF /dsn/2ui5_t_91 WITH EMPTY KEY.
  ...
  METHOD save.
    ...
    DATA(ls_db) = VALUE /dsn/2ui5_t_91(
        uname   = uname
        handle  = handle
        handle2 = handle2
        handle3 = handle3
        data    = /dsn/2ui5_cl_util=>xml_stringify( data ) ).

    IF lv_id IS NOT INITIAL.
      ls_db-id = lv_id.
    ELSE.
      ls_db-id = /dsn/2ui5_cl_util=>uuid_get_c32( ).
    ENDIF.

    MODIFY /dsn/2ui5_t_91 FROM @ls_db.
    ASSERT sy-subrc = 0.
